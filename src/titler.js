...
// marker takes. Anything past that is not a title, whatever wrote it.
const PROMPT_TITLE_MAX = MAX_TITLE_CHARS + DONE_MARKER.length;

// Titles reaching a prompt are not necessarily ours. A custom-title record is a line in a file the
// desktop app and any other tool writes into, so its text is untrusted input, and interpolated
// verbatim a newline plus an imperative sentence reads to the model as a rule of the prompt rather
// than as data. Flattening every line break and control character to a single space takes that away,
// and the cap bounds how much of the prompt a title can be. This is the prompt boundary only -
// nothing here changes what gets written to a transcript.
const sanitizeForPrompt = (title) => {
  if (typeof title !== 'string') return '';
  return [...title
    .replace(/\u001b\[[0-9;]*[A-Za-z]/g, '')
    .replace(/[\s\u0000-\u001f\u007f]+/g, ' ')
    .trim()]
    .slice(0, PROMPT_TITLE_MAX)
    .join('')
    .trim();
};
...
function buildDonePrompt({ currentTitle, excerpt = '' }) {
  ...
  // Same boundary as the title prompts: the string here came off a transcript record, so it is
  // flattened and bounded before it can read as a rule rather than as the session's name.
  const safeTitle = sanitizeForPrompt(currentTitle);
  return `${DONE_PROMPT_SIGNATURE}

Session title: ${safeTitle || '(none)'}
