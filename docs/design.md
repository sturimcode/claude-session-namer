...
- **Known limitation: the drift gate counts user turns, not work done.** A heavily agentic session - a handful of prompts and hours of tool calls - grows a long way without its user-turn count moving, so it re-checks rarely, and a correction the user types mid-session may not be read until the count doubles again. The gate is unchanged: counting user turns is what makes calls scale with the log of a conversation rather than with transcript size, and every alternative trigger costs model calls on sessions that have not changed topic. A second trigger (elapsed assistant turns, transcript bytes) is candidate work for a later version, listed here so the trade-off is on the record rather than rediscovered.
