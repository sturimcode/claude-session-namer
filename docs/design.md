...
- Per session: last-check user-turn count, last-check transcript record count, titles written by the tool, manual flag (set only by `rename` and `protect`)
